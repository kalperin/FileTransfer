...
{	
    ...
    /* Initialize and assign listeners to UI Components */
	...
	/*
	 * Initialize the File Transfer Component and its listeners
	 */
	private void initializeFileTransferComponent()
	{
		//Construct the component using the Bus Attachment and Session Id created by the AllJoyn Manager
		ftComponent = new FileTransferComponent(ajManager.getBusAttachment(), ajManager.getSessionId());
		
		//Register announcement listener - create a toast when a file announcement is received
		ftComponent.setFileAnnouncementReceivedListener(new FileAnnouncementReceivedListener()
		{
		...
		ftComponent.setUnannouncedFileRequestListener(new UnannouncedFileRequestListener()
		{			
	...
	private void onHostButtonClicked()
	{
		...
		initializeFileTransferComponent();		
	}
	...
	private void OnUnshareButtonClicked()
	{
		...
		final ArrayList<FileDescriptor> announcedFiles = ftComponent.getAnnouncedLocalFiles();
	...
	private void onRequestButtonClicked()
	{
		...
		final ArrayList<FileDescriptor> availableFiles = ftComponent.getAvailableRemoteFiles();
	...
	private void monitorReceiveProgress()
	{
		monitorReceiveThread = new Thread(new Runnable()
		{
			public void run()
			{
				...
				ArrayList<ProgressDescriptor> receiveList = ftComponent.getReceiveProgressList();
				...
				while (receiveList.size() > 0)
				{
					...
					receiveList = ftComponent.getReceiveProgressList();					
				}				
	...
	private void monitorSendProgress()
	{
		monitorSendThread = new Thread(new Runnable()
		{
			public void run()
			{
				...
				ArrayList<ProgressDescriptor> sendList = ftComponent.getSendingProgressList();
				...
				while (sendList.size() > 0)
				{
					...
					sendList = ftComponent.getSendingProgressList();					
				}				
	...
	private void onPauseReceiveButtonClicked()
	{
		...
		ftComponent.pauseFile(descriptor.fileID);
	...
	private void onCancelReceiveButtonClicked()
	{
		...
		ftComponent.cancelReceivingFile(descriptor.fileID);		
	}
	...
	private void onPauseSendButtonClicked()
	{
		...
		ftComponent.cancelSendingFile(descriptor.fileID);		
	}
